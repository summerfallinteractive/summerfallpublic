...
RegisterNetEvent('sheriff-badge:badgeanim')
AddEventHandler('sheriff-badge:badgeanim', function(prop_name)
		...
		local boneIndex = GetPedBoneIndex(playerPed, 28422)
			if IsPedInAnyVehicle(playerPed, false) then
				if GetDistanceBetweenCoords(playerCoords, playerCoords2, true) <= Config.Distance then
					SendNUIMessage({ action = "open", array = image})
				end
			else
...
RegisterNetEvent('sheriff-badge:open')
AddEventHandler('sheriff-badge:open', function(who, image)
